...
def create_sample_spreadsheet():
    ...
    print("5. Make sure the spreadsheet is publicly readable (or shared with your service account)")
...
def test_connection():
    ...
    try:
        ...
        try:
            ...
            print(f"   Spreadsheet title: {info.get('properties', {}).get('title', 'Unknown')}")
            return True
        except Exception as e:
            print(f"❌ Cannot access spreadsheet: {e}")
            print("   Make sure the spreadsheet ID is correct and the sheet is accessible")
            return False
