...
from weekly_scheduler import (
    acquire_scheduler_lock, release_scheduler_lock, WeeklyLeadGenerator
)
...
class TestWeeklyScheduler:
    ...
    def test_weekly_lead_generator_init(self):
        ...
        with patch('weekly_scheduler.CONFIG', {'opportunity_tracking_file': 'test.json'}):
    ...
    def test_load_tracking_data_existing_file(self):
        ...
        try:
            with patch('weekly_scheduler.CONFIG', {'opportunity_tracking_file': temp_file}):
                generator = WeeklyLeadGenerator()
    ...
    def test_load_tracking_data_missing_file(self):
        ...
        with patch('weekly_scheduler.CONFIG', {'opportunity_tracking_file': 'nonexistent.json'}):
            generator = WeeklyLeadGenerator()
    ...
    def test_cleanup_old_data(self):
        ...
        with patch('weekly_scheduler.CONFIG', {
            ...
            generator.opportunities_tracking['opportunities_by_week'][old_date.strftime('%Y-%W')] = 10
    ...
    def test_process_results(self, mock_read_csv):
        ...
        mock_df["Signal Type"].value_counts.return_value.to_dict.return_value = {1: 10, 2: 15}
    ...
    def test_send_weekly_report_success(self, mock_smtp):
        ...
        with patch('weekly_scheduler.CONFIG', {
            ...
            with patch.object(generator.credentials_manager, 'get_email_config') as mock_email_config:
    ...
    def test_run_weekly_job_success(self):
        ...
        with patch('weekly_scheduler.CONFIG', {
            ...
            with patch.object(generator, 'cleanup_old_data') as mock_cleanup, \
                 patch.object(generator, 'run_lead_generation', return_value=True) as mock_run, \
                 patch.object(generator, 'send_weekly_report') as mock_send:
    ...
    def test_run_weekly_job_failure(self):
        ...
        with patch('weekly_scheduler.CONFIG', {
            ...
            with patch.object(generator, 'cleanup_old_data') as mock_cleanup, \
                 patch.object(generator, 'run_lead_generation', return_value=False) as mock_run, \
                 patch.object(generator, 'send_weekly_report') as mock_send:
