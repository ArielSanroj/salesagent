...
from models import Opportunity, Article, SearchQuery
from constants import SIGNAL_TYPES, EMAIL_TEMPLATE_MAPPING, DEFAULT_KEYWORDS
...
Email:"""
    ...
    def process_article(self, article: Dict[str, Any], signal_type: int) -> Optional[Opportunity]:
        ...
        try:
            ...
            opportunity = Opportunity(
                ...
                email=email,
                url=url,
                date=self.parse_article_date(date) or datetime.now().strftime("%Y-%m-%d"),
    ...
    def process_signal(self, signal_id: int, max_results: int = 10) -> List[Opportunity]:
        """Process a specific signal type and return opportunities"""
        logger.info(f"Processing signal {signal_id}: {SIGNAL_TYPES.get(signal_id, 'Unknown')}")
        ...
        if self.search_service:
            for query in queries:
                articles = self.search_service.search_articles(query, max_results // len(queries))
