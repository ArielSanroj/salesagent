...
class ScrapingService:
    ...
    def can_scrape(self, url: str) -> bool:
        ...
        try:
            ...
            logger.warning(f"Failed to fetch robots.txt for {url}: Status {response.status_code}")
            return True  # Assume allowed if can't check
            
        except Exception as e:
            logger.warning(f"Failed to check robots.txt for {url}: {e}, assuming scrape allowed")
            return True
    ...
    def scrape_url_content(self, url: str) -> Optional[Dict[str, str]]:
        ...
        for attempt in range(SCRAPING_MAX_RETRIES):
            try:
                ...
                response = self.session.get(url, headers=headers, timeout=SCRAPING_TIMEOUT)
                ...
                if attempt < SCRAPING_MAX_RETRIES - 1:
                    ...
                    raise NetworkError(f"Failed to scrape {url} after {SCRAPING_MAX_RETRIES} attempts: {e}")
            except Exception as e:
