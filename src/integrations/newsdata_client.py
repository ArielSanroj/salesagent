#!/usr/bin/env python3
"""
NewsData.io API Client for HR Tech Lead Generation System
Optimized client with retry logic and rate limiting
"""
...
from constants import NEWS_API_MAX_RETRIES, NEWS_API_TIMEOUT, NEWSDATA_LATEST_URL
...
class NewsDataClient:
    """Optimized NewsData.io API client"""

    def __init__(self, api_key: str, call_limit: int = 50):
    ...
    def _check_rate_limit(self) -> bool:
        ...
        if self.call_count >= self.call_limit:
            logger.warning(
                f"NewsData.io daily call limit reached: {self.call_count}/{self.call_limit}"
            )
            return False
    ...
    def search_articles(
        ...
        try:
            while len(aggregated) < num_results and page_count < max_pages:
                ...
                response = self.session.get(
                    endpoint, params=request_params, timeout=NEWS_API_TIMEOUT
                )
        ...
        logger.info(
            f"NewsData.io returned {len(aggregated)} articles for query '{query}'"
        )
    ...
    def get_remaining_calls(self) -> int:
        ...
        return max(0, self.call_limit - self.call_count)
