...
class Opportunity:
    ...
    def __init__(self, title: str, company: str, person: str, email: str, 
                 url: str, date: str, content: str, relevance_score: float, 
                 signal_type: int, source: str):
        self.title = title
...
def calculate_relevance_score(content: str, keywords: List[str]) -> float:
    ...
    total_occurrences = sum(content_lower.count(keyword.lower()) for keyword in keywords)
...
def extract_company_name(text: str) -> Optional[str]:
    ...
    if not llm_service:
        logger.warning("LLM service not available, using fallback for company extraction")
        return None
...
def extract_person_name(text: str) -> Optional[str]:
    ...
    # Fallback if LLM service is not available
    if not llm_service:
        logger.warning("LLM service not available, using fallback for person extraction")
        return None
        ...
        if response and response != "Service temporarily unavailable":
            ...
            if '@' in response:
                ...
                if '@' in email and '.' in email and is_valid_email(email):
                    logger.info(f"LLM suggested email for {person} at {company}: {email}")
                    return email
...
def fetch_news_articles(query: str, num_results: int = 10, domains: Optional[List[str]] = None) -> List[Dict[str, Any]]:
...
def process_opportunity(article: Dict[str, Any], signal_type: int) -> Optional[Opportunity]:
    ...
    try:
        ...
        logger.info(f"Processed opportunity: {company} - {person} (Score: {relevance_score:.2f})")
        return opportunity
...
def run_signal(signal_id: int) -> List[Opportunity]:
    ...
    logger.info(f"Signal {signal_id} completed: {len(opportunities)} opportunities found")
...
def filter_results(opportunities: List[Opportunity]) -> List[Opportunity]:
    ...
    logger.info(f"Filtered {len(opportunities)} opportunities to {len(filtered)} unique opportunities")
    return filtered
...
def test_signal(signal_id: int) -> List[Opportunity]:
    ...
    if opportunities:
        ...
        logger.info(f"Test completed: {len(opportunities)} opportunities saved to {filename}")
        return opportunities
    else:
...
def send_email_report(report_content: str, csv_file_path: Optional[str] = None) -> bool:
    ...
    try:
        ...
        msg["Subject"] = f"HR Tech Lead Generation Report - {datetime.now().strftime('%Y-%m-%d %H:%M')}"
...
def main() -> None:
    ...
    try:
        ...
        if is_weekly_run:
            logger.info(f"Starting weekly run - Target: {target_opportunities} opportunities")
            ...
            logger.info(f"Weekly run completed: {len(filtered_opportunities)} opportunities generated")
